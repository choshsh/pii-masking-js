...
export function maskPhone(phone: string, maskChar: string = '*'): string {
  ...
  if (len === 11) {
    ...
    maskedP2 = p2.substring(0, 2) + maskChar.repeat(2);
    maskedP3 = p3.substring(0, 2) + maskChar.repeat(2);
  } else if (len === 10) {
    // 지역번호 3자리 + 국번 3자리 (031-123-5678)
    p1 = numbers.substring(0, 3);
    p2 = numbers.substring(3, 6);
    p3 = numbers.substring(6, 10);
    maskedP2 = p2.substring(0, 2) + maskChar.repeat(1);
    maskedP3 = p3.substring(0, 2) + maskChar.repeat(2);
  } else if (len === 9) {
    ...
    maskedP2 = p2.substring(0, 1) + maskChar.repeat(2);
    maskedP3 = p3.substring(0, 2) + maskChar.repeat(2);
  } else if (len === 8) {
    // 대표번호 (e.g. 1588-1234)
    const part1 = numbers.substring(0, 4);
    const part2 = numbers.substring(4, 8);
    const maskedPart2 = part2.substring(0, 2) + maskChar.repeat(2);
    if (hasHyphen) {
